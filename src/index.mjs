...
class MCBot {
  ...
  async executeCommand(command) {
    ...
    switch (cmd) {
      ...
      case "info":
        ...
        this.logInfo(`    Position:       ${`X: ${chalk.hex("#de495b")(JSON.stringify(Math.round(this.bot.entity.position.x)), chalk.white("Y:"), Math.round(this.bot.entity.position.y), chalk.white("Z:"), Math.round(this.bot.entity.position.z))}`}`);
        break;
      ...
      case "pos":
        this.logInfo(`My current Position: ${`X: ${chalk.hex("#de495b")(JSON.stringify(Math.round(this.bot.entity.position.x)), chalk.white("Y:"), Math.round(this.bot.entity.position.y), chalk.white("Z:"), Math.round(this.bot.entity.position.z))}`}`);
        break;
  ...
  initEvents() {
    ...
    this.bot.on("playerCollect", (collector, collected) => {
      this.logInfo(`${chalk.hex("#b052f2")(JSON.stringify(collector.username))} picked up`, chalk.hex("#cfa1f0")(JSON.stringify(collected.metadata[8]?.itemCount)), chalk.hex("#cfa1f0")(JSON.stringify(collected.displayName)), "at ", "X:", chalk.hex("#de495b")(JSON.stringify(Math.round(collected.position.x)), chalk.white("Y:"), Math.round(collected.position.y), chalk.white("Z:"), Math.round(collected.position.z)));
      this.autoTotem();
    });
